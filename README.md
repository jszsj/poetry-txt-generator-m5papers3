...
```json
{
    ...
    "text_conversion": "none",              // 简繁转换: none(不转换) | s2t(简→繁) | t2s(繁→简) | s2tw(简→台湾) | tw2s(台湾→简)
...
支持自动简繁体转换，配置 `text_conversion` 参数：
...
PS：由于香港、台湾等地习惯不同，简繁转换较为复杂，目前仅简单实现，结果中可能会出现字词不符合上下文的情况，如果要求严格请自行斟酌使用。
